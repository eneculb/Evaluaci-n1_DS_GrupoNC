...
def fig1_particion_modal():
    ...
    fig.suptitle("FIGURA 1 — Partición Modal",
    ...
    ax.axhline(media_d, color="#ffd93d", ls="--", lw=1.5,
               label=f"Media = {media_d:.2f} km")
    ...
    # 1d. Viajes vs. ponderados por distancia
    ax = axes[1, 1]
...
def fig2_tiempos_genero():
    ...
    fig.suptitle("FIGURA 2 — Tiempos de Viaje y Tasa por Género",
                 fontsize=13, color="#e0e0e0", fontweight="bold")
    ...
    ax.legend(handles=[Patch(color="#ff6b6b", label="Periferia"),
                       Patch(color="#667eea", label="Centro/Oriente")], fontsize=8)
    ax.grid(axis="x")

    # 2b. Tasa por género
    ax = axes[1]
    bars2 = ax.bar(df_genero["genero"], df_genero["tasa"],
                   color=["#f093fb", "#667eea", "#4ecdc4"], width=0.5)
    for b in bars2:
...
def fig3_correlacion():
    ...
    labels      = ["Hombre", "Edad", "Teletrabajo", "Ingreso",
                   "Modo", "Propósito", "T. Viaje", "Viajes/día"]

    fig, ax = plt.subplots(figsize=(9, 7))
    fig.suptitle("FIGURA 3 — Matriz de Correlación de Variables de Movilidad",
...
def fig4_comparativa_modelos(resultados: dict):
    ...
    info = [
        ("Regresión Lineal",  modelos["lr"].predict(X_test), "#667eea"),
        ("Árbol de Decisión", modelos["dt"].predict(X_test), "#4ecdc4"),
        ("Random Forest",     modelos["rf"].predict(X_test), "#ffd93d"),
    ]

    fig, axes = plt.subplots(1, 3, figsize=(17, 5))
    fig.suptitle("FIGURA 4 — Comparativa de Modelos Predictivos (Test Set)",
                 fontsize=13, color="#e0e0e0", fontweight="bold")
...
def fig5_importancia_escenarios(resultados: dict):
    ...
    fig.suptitle("FIGURA 5 — Importancia de Variables y Predicción de Escenarios",
                 fontsize=13, color="#e0e0e0", fontweight="bold")
    ...
    bars_f   = ax.barh(labels_f, vals_f, color=[PALETTE[i % len(PALETTE)] for i in range(len(vals_f))])
    ...
    ax.set_ylabel("Viajes diarios predichos")
